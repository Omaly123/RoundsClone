	...
	if collision_info:
		...
		if collider != null && collider.has_method("fire") :
			if collider.blocking >=1 :
				for card in cards:
					card.hit_blocked_effect(collider)
				velocity = velocity.bounce(collision_info.get_normal())
			else: if collider.blocking <=0:
				#collider.hit.rpc(damage, shooter)
				collider.hit(damage, shooter)
				for card in cards:
					card.hit_effect(collider, damage)
					pass
				queue_free()
		else: if bounces == 0:
			for card in cards:
				card.last_wall_hit_effect(collision_info)
			queue_free()
		else: if bounces > 0 : 
			bounces-=1
			for card in cards:
				card.wall_hit_effect(collision_info)
			velocity = velocity.bounce(collision_info.get_normal())
